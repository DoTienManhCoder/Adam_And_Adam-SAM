...
def main():
    ...
    print('\n=== Huấn luyện với Adam ===')
    model_adam = MLP(hidden_dims=hidden_dims, dropout=dropout).to(device)
    ...
    model_sam = MLP(hidden_dims=hidden_dims, dropout=dropout).to(device)
...
def plot_results(results, epochs):
    ...
    plt.tight_layout()
    plt.savefig('mlp_comparison.png', dpi=300, bbox_inches='tight')
    print('\nĐã lưu biểu đồ: mlp_comparison.png')
    plt.show()
